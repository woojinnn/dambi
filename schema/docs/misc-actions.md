...
{
  "_kind": "wrap",
  "nativeAsset":  { "kind": "native", "chainId": 1, "symbol": "ETH" },
  "wrappedAsset": { "kind": "erc20",  "chainId": 1, "address": "0xC02a…", "symbol": "WETH", "decimals": 18 },
...
```jsonc
{
  ...
  "approvalKind":  "erc20",
  "isUnlimited":   true,                              // ← derived 편의 필드
...
| **`amount.kind`** | `unlimited` 면 무한 위임 — 정책상 거의 항상 차단 권장 |
| `approvalKind` | 어떤 표준의 approve 인가 |
| `isUnlimited` | `amount.kind === 'unlimited'` 의 derived bool. 정책 DSL 의 편의 |
| `currentAllowance` | 이미 얼마나 줘있는지 (host:onchain 단일 lookup). 증가 폭 정책 ("기존의 10% 이상 증가 금지") |
| `validity` | Permit2 한정 — `source="grant-expiration"`, 부여된 allowance 가 `expiresAt` 까지 유지. 일반 ERC-20/721/1155 approve 는 만료 개념 없음 — omit. 정책 "expiresInSeconds > 1y 차단" |
...
validity == null || (
  ...
  validity.expiresInSeconds <= 604800             // 7 days
)
...
- `schema/common/_common.json` (AssetRef, AmountConstraint, UsdValuation)
