...
| `add_liquidity` | `dex` (V2/V3/V4/Balancer/Curve), `yield` (Pendle SY 발행, vault deposit) |
| `remove_liquidity` | `dex` (모든 DEX), `yield` (vault withdraw, Pendle redeem) |
...
| 필드 | 뜻 |
|---|---|
| `action` | 의미 단위 식별자. 정책 1차 분기점. v1.0.1: `swap`/`add_liquidity`/`remove_liquidity`/`wrap`/`unwrap`/`approve`. fields._kind 와 동일. |
...
| `fields` | action 별 schema (`actions/<action>.json`) 로 추가 검증되는 객체. 반드시 `_kind` 필드를 가지며 `_kind === action`. |
...
- `schema/common/_common.json` — Address / DecimalString / AmountConstraint / AssetRef / UsdValuation primitive
