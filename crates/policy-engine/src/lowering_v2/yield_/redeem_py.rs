...
use policy_transition::action::yield_::RedeemPyAction;
...
pub(crate) fn lower(
    ...
    if let Some(nano) = action
        .external_token
        .as_ref()
        .and_then(|t| ctx.amount_nano(t, action.min_output))
    {
        m.insert("minOutputNano".into(), Value::from(nano));
    }
...
mod tests {
    ...
    use policy_transition::action::yield_::{RedeemPyAction, YieldAction};
