...
fn decode_v4_swap(
    ...
    for step in &steps {
        let env = match step.opcode {
            ...
            V4_ACTION_SWAP_EXACT_OUT_SINGLE => v4_swap_exact_out_single(ctx, step, validity.clone())?,
...
fn v4_params_tuple(step: &abi_resolver::subdecode::opcode_stream::DecodedStep)
...
fn v4_swap_exact_in_single(
    ...
    Ok(swap_envelope(SwapAction {
        ...
        token_in: v4_asset_ref(ctx, &token_in),
        token_out: v4_asset_ref(ctx, &token_out),
        amount_in: amount_constraint(AmountKind::Exact, decimal(&amount_in.to_string())?),
        amount_out: amount_constraint(AmountKind::Min, decimal(&amount_out_min.to_string())?),
...
fn v4_swap_exact_in_multi(
    ...
    Ok(swap_envelope(SwapAction {
        mode: SwapMode::ExactIn,
        token_in: v4_asset_ref(ctx, &currency_in),
        token_out: v4_asset_ref(ctx, &currency_out),
        amount_in: amount_constraint(AmountKind::Exact, decimal(&amount_in.to_string())?),
        amount_out: amount_constraint(AmountKind::Min, decimal(&amount_out_min.to_string())?),
...
fn v4_swap_exact_out_single(
    ...
    Ok(swap_envelope(SwapAction {
        ...
        token_in: v4_asset_ref(ctx, &token_in),
        token_out: v4_asset_ref(ctx, &token_out),
        amount_in: amount_constraint(AmountKind::Max, decimal(&amount_in_max.to_string())?),
        amount_out: amount_constraint(AmountKind::Exact, decimal(&amount_out.to_string())?),
        recipient: ctx.from.clone(),
        validity,
        fee_bps: extract_pool_fee_bps(&pk)?,
        enrichment: SwapEnrichment::default(),
...
fn native_asset(ctx: &CallContext<'_>) -> AssetRef {
    AssetRef {
        kind: AssetKind::Native,
        chain_id: ctx.chain_id,
        address: None,
