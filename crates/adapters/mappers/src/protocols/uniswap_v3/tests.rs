...
fn erc20(_chain_id: u64, address: &str, symbol: Option<&str>, decimals: Option<u8>) -> AssetRef {
    AssetRef {
    ...
    DecodedCall {
        ...
        args: vec![
            DecodedArg {
                ...
                value: DecodedValue::Address(address("0xdac17f958d2ee523a2206206994597c13d831ec7")),
            },
            DecodedArg {
                name: "tokenOut".to_owned(),
                abi_type: "address".to_owned(),
                value: DecodedValue::Address(address("0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2")),
            },
            DecodedArg {
                name: "fee".to_owned(),
            ...
            DecodedArg {
                ...
                abi_type: "address".to_owned(),
                value: DecodedValue::Address(address("0x1111111111111111111111111111111111111111")),
            },
            DecodedArg {
    ...
    ActionEnvelope {
        ...
        action: Action::Swap(SwapAction {
            ...
            input_token: AssetRefWithAmountConstraint {
                ...
                amount: amount(AmountKind::Exact, "1000000"),
            },
            output_token: AssetRefWithAmountConstraint {
                asset: erc20(
                    1,
...
fn test_map_exact_input_parses_path_and_uses_first_hop_fee() {
    ...
    assert_eq!(
        swap.input_token.asset.address,
        Some(address("0xdac17f958d2ee523a2206206994597c13d831ec7"))
    );
    assert_eq!(
        swap.output_token.asset.address,
...
fn maps_exact_output_single_to_swap_action() {
    ...
    assert_eq!(
        swap.input_token.amount,
        amount(AmountKind::Max, "200000000")
    );
    assert_eq!(
        swap.output_token.amount,
        amount(AmountKind::Exact, "100000000")
    );
    assert_eq!(
        swap.input_token.asset.address,
        Some(address("0xdac17f958d2ee523a2206206994597c13d831ec7"))
