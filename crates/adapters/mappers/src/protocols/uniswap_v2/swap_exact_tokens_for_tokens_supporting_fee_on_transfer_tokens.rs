//! `swapExactTokensForTokensSupportingFeeOnTransferTokens` — V2 Router02
//! variant for fee-on-transfer ERC-20s. Same calldata shape as
//! `swapExactTokensForTokens`; only the decoder_id and selector differ.

use abi_resolver::ids::SWAP_EXACT_TOKENS_FOR_TOKENS_FOT_DECODER_ID;
use abi_resolver::{DecodedCall, DecoderId};
use policy_engine::action::dex::{SwapAction, SwapMode};
use policy_engine::action::{ActionEnvelope, AmountKind};

use crate::mapper::{MapContext, Mapper, MapperError, MapperId};

use super::common::{
    address_arg, address_array_arg, amount_constraint, asset_with_amount, path_assets,
    swap_envelope, uint_arg, validity,
};

pub const SWAP_EXACT_TOKENS_FOR_TOKENS_FOT_MAPPER_ID: &str =
    "uniswap-v2/swapExactTokensForTokensSupportingFeeOnTransferTokens";

#[derive(Debug, Clone, Copy, Default)]
pub struct SwapExactTokensForTokensFotMapper;

impl SwapExactTokensForTokensFotMapper {
    #[must_use]
    pub const fn new() -> Self {
        Self
    }
}

impl Mapper for SwapExactTokensForTokensFotMapper {
    fn id(&self) -> MapperId {
        MapperId::new(SWAP_EXACT_TOKENS_FOR_TOKENS_FOT_MAPPER_ID)
    }

    fn accepts(&self, decoded: &DecodedCall) -> bool {
        decoded.decoder_id == DecoderId::new(SWAP_EXACT_TOKENS_FOR_TOKENS_FOT_DECODER_ID)
    }

    fn map(
        &self,
        ctx: &MapContext<'_>,
        decoded: &DecodedCall,
    ) -> Result<Vec<ActionEnvelope>, MapperError> {
        let amount_in = uint_arg(decoded, "amountIn")?;
        let amount_out_min = uint_arg(decoded, "amountOutMin")?;
        let path = address_array_arg(decoded, "path")?;
        let recipient = address_arg(decoded, "to")?;
        let deadline = uint_arg(decoded, "deadline")?;
        let (token_in, token_out) = path_assets(ctx, &path)?;

        Ok(vec![swap_envelope(SwapAction {
            swap_mode: SwapMode::ExactIn,
            input_token: asset_with_amount(
                token_in,
                amount_constraint(AmountKind::Exact, amount_in)?,
            ),
            output_token: asset_with_amount(
                token_out,
                amount_constraint(AmountKind::Min, amount_out_min)?,
            ),
            recipient,
            validity: Some(validity(deadline)?),
            fee_bps: Some(30),
        })])
    }
}
