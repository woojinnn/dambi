//! `swapExactTokensForETHSupportingFeeOnTransferTokens` — V2 Router02 variant
//! for fee-on-transfer input tokens, ETH-output.

use abi_resolver::ids::SWAP_EXACT_TOKENS_FOR_ETH_FOT_DECODER_ID;
...
use super::common::{
    address_arg, address_array_arg, amount_constraint, asset_with_amount, native_eth_asset_ref,
    path_first_asset, swap_envelope, uint_arg, validity,
};

pub const SWAP_EXACT_TOKENS_FOR_ETH_FOT_MAPPER_ID: &str =
    "uniswap-v2/swapExactTokensForETHSupportingFeeOnTransferTokens";

#[derive(Debug, Clone, Copy, Default)]
pub struct SwapExactTokensForETHFotMapper;

impl SwapExactTokensForETHFotMapper {
    #[must_use]
...
impl Mapper for SwapExactTokensForETHFotMapper {
    fn id(&self) -> MapperId {
        MapperId::new(SWAP_EXACT_TOKENS_FOR_ETH_FOT_MAPPER_ID)
    }

    fn accepts(&self, decoded: &DecodedCall) -> bool {
        decoded.decoder_id == DecoderId::new(SWAP_EXACT_TOKENS_FOR_ETH_FOT_DECODER_ID)
    }
