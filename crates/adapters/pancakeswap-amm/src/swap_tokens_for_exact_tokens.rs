//! PancakeSwap V2 Router `swapTokensForExactTokens(uint256 amountOut,
...
/// Selector for `swapTokensForExactTokens`.
pub const SELECTOR: [u8; 4] = swapTokensForExactTokensCall::SELECTOR;

/// Decoded `swapTokensForExactTokens` parameters.
...
/// ABI-encode `swapTokensForExactTokens` calldata.
#[must_use]
...
/// Decode `swapTokensForExactTokens` calldata.
///
/// # Errors
///
...
/// Adapter for `swapTokensForExactTokens`.
#[derive(Debug)]
pub struct Adapter_ {
...
impl Adapter for Adapter_ {
    fn id(&self) -> AdapterId {
        static_adapter_id("pancakeswap-amm/swapTokensForExactTokens@0.1.0")
    }
    ...
    fn build(&self, tx: &TransactionRequest) -> Result<Action, AdapterError> {
        ...
        Ok(dex_swap_action(
            ...
            &recipient_addr,
            Some(25),
            "pancakeswap-amm/swapTokensForExactTokens",
        ))
    }
}
