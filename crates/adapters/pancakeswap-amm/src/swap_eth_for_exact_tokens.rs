//! PancakeSwap V2 Router `swapETHForExactTokens(uint256 amountOut,
//! address[] path, address to, uint256 deadline)`. Payable — `msg.value` is
//! the maximum native BNB the caller is willing to spend.
...
impl Adapter for Adapter_ {
    fn id(&self) -> AdapterId {
        static_adapter_id("pancakeswap-amm/swapETHForExactTokens@0.1.0")
    }
    ...
    fn build(&self, tx: &TransactionRequest) -> Result<Action, AdapterError> {
        ...
        Ok(dex_swap_action(
            ...
            &recipient_addr,
            Some(25),
            "pancakeswap-amm/swapETHForExactTokens",
        ))
    }
}
...
mod tests {
    ...
    fn build_uses_msg_value_as_input_amount_max() {
        ...
        let tx = TransactionRequest {
            ...
            value_wei: "1500000000000000000".into(), // 1.5 BNB max
