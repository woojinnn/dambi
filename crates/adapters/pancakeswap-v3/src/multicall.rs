//! PancakeSwap V3 SwapRouter `multicall` expansion.
//!
//! Structurally identical to Uniswap V3's multicall: each supported child
//! calldata is decoded into a leaf `Action::Dex` by the matching per-function
//! adapter, then all leaves are merged into a single aggregate by
//! `merge_dex_actions`.
...
pub struct Params {
    /// Optional multicall deadline (set when the deadline-overload is used).
    pub deadline: Option<alloy_primitives::U256>,
    /// Child calldata payloads (one per nested call).
    pub data: Vec<Vec<u8>>,
}
...
/// Adapter for PancakeSwap V3 SwapRouter multicall expansion.
#[derive(Debug)]
pub struct Adapter_ {
...
impl Adapter_ {
    /// Construct an adapter with BSC PancakeSwap V3 SwapRouter and child adapters.
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    fn expand_calls(
        ...
        for child_data in calls {
            ...
            if selector == SELECTOR_NO_DEADLINE || selector == SELECTOR_DEADLINE {
                ...
                out.push(
                    <exact_input_single::Adapter_ as TypedAdapter>::build_action(
...
impl Adapter for Adapter_ {
    fn id(&self) -> AdapterId {
        static_adapter_id("pancakeswap-v3/multicall@0.1.0")
    }
...
mod tests {
    ...
    #[test]
    fn build_merges_two_swaps_into_one_dex_action() {
        let adapter = Adapter_::new();
        let action = adapter
            .build(&tx(encode_deadline(
                U256::from(1u64),
                vec![swap(50_000_000_000_000_000), swap(200_000_000_000_000_000)],
            )))
            .unwrap();
        match action {
            Action::Dex(d) => {
