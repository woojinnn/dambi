//! PancakeSwap V3 SwapRouter `exactOutput` — multi-hop, exact-out. The packed
//! path is reversed for exact-out semantics (last token is `tokenIn`,
//! first is `tokenOut`).
...
pub struct Params {
    /// Packed V3 path. For exact-out: `[tokenOut][fee0][...][tokenIn]`.
    pub path: Vec<u8>,
    /// Recipient address.
...
/// Adapter for PancakeSwap V3 `exactOutput`.
#[derive(Debug)]
pub struct Adapter_ {
...
impl Adapter for Adapter_ {
    fn id(&self) -> AdapterId {
        static_adapter_id("pancakeswap-v3/exactOutput@0.1.0")
    }
...
mod tests {
    ...
    fn sample_params() -> Params {
        Params {
            ...
            amount_in_maximum: U256::from(4_000_000_000_000_000_000_u128),
        }
    }
    ...
    fn build_handles_reversed_path_correctly() {
        ...
        match adapter.build(&tx).unwrap() {
            Action::Dex(d) => {
                ...
                // Path layout [WBNB, fee, USDT] but exact-out reads it
                // reversed: input = USDT, output = WBNB.
