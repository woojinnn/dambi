...
//! `&dyn WalletStore` so the actual backend — in-memory for dev/test,
//! `SQLite` for production — is interchangeable. The trait lives here in
//! `simulation-state` (not in `simulation-sync` or `simulation-db`) so both
//! the `SQLite` impl (db crate) and the consumers (sync, server) can depend
//! on it without forming a dependency cycle.
...
///
/// Three operations, all async because the `SQLite` backend runs blocking IO
/// on a tokio blocking pool. Implementations must be `Send + Sync` so they
/// can sit behind an `Arc` in a multi-threaded axum server.
///
/// Contract:
