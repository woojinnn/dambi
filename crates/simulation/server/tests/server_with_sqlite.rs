//! Integration: the axum server backed by per-user `SqliteWalletStore`s.
//!
...
//! requests (the load→reduce→save path runs through SQLite, not memory).
//!
...
/// Build an AppState backed by tempdir-backed real SQLite stores.
...
#[tokio::test]
async fn sqlite_backed_evaluate_persists_state_across_requests() {
