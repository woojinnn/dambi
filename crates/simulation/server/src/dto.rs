...
// `ExecutionReportRequest` / `ExecutionReportOutcome` / `ExecutionReportResponse`
// were removed: post-policy lifecycle facts now live in
// `chrome.storage.local` via the browser extension's
// `execution-report-storage.ts`. The server no longer has an
// `/execution-report` route to deserialize them.
...
mod tests {
    ...
    // `execution_report_outcomes_round_trip` was removed alongside the
