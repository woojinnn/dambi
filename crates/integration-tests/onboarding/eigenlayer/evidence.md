...
| Claude Code or sub-agent research executed | done | sub-agent Zeno 019e891c-83d0-7c73-b4cd-6592e5da6564 completed read-only P0/P2 audit; findings integrated: strict corpus + EIGEN/bEIGEN token gap + defer share |
...
| Codex-only candidates listed | done | Codex candidates came from registryV2/surface/eigenlayer/_deployments.json, coverage JSON, manifests, corpus, Etherscan and Dune artifacts |
| Claude/sub-agent-only candidates listed | done | sub-agent Zeno 019e891c-83d0-7c73-b4cd-6592e5da6564 completed read-only P0/P2 audit; findings integrated: strict corpus + EIGEN/bEIGEN token gap + defer share |
...
| final contract inventory verified against first-party sources | done | registryV2/surface/eigenlayer/_deployments.json verified as inventory source for representative chain scope |
| pool-heavy/factory protocol address universe source/query/count recorded, or explicitly not applicable | done | not pool/factory/vault-heavy for address-universe gate; representative chain mainnet only; deployments file has 24 contracts, 4 cover, 20 exclude |
| pool-heavy/factory universe artifact is machine-readable, nonzero, and committed, or explicitly not applicable | done | not pool/factory/vault-heavy for address-universe gate; representative chain mainnet only; deployments file has 24 contracts, 4 cover, 20 exclude |
| every pool/factory child address in universe dispositioned as cover/exclude/defer with reason and batch boundary | done | not pool/factory/vault-heavy for address-universe gate; representative chain mainnet only; deployments file has 24 contracts, 4 cover, 20 exclude |
...
| token-surface inventory completed or explicitly scoped out | done | Added EIGEN and bEIGEN ERC20 token artifacts for standard ERC adapter enumeration |
| `registryV2/surface/<protocol>/_deployments.json` updated if applicable | done | registryV2/surface/eigenlayer/_deployments.json retained; Balancer also gained _address_universe.json |
| `npm run check:surface` output recorded | done | `cd registryV2 && npm run check:surface -- --protocol eigenlayer` PASS after edits; target I0 row: 24 deployed, 4 cover, 20 exclude |

## P1 Authoring Evidence

| required evidence | status | artifact / exact command / summary |
|---|---|---|
| every COVER selector mapped to existing ActionBody or Tier3 requirement | done | COVER selector mapping recorded in registryV2/surface/eigenlayer/*.coverage.json and registryV2/manifests/eigenlayer/**/*.json; no new ActionBody domain needed |
...
| manifest files added/changed listed | done | Existing manifest set under registryV2/manifests/eigenlayer/; this PR changes corpus/evidence/token/universe only, no manifest semantics changed |
...
| required evidence | status | artifact / exact command / summary |
|---|---|---|
| fuzz command with seed recorded | done | cargo run -p policy-engine-integration-tests --bin v3-harness -- fuzz --filter eigenlayer --iterations 5000 --seed 0x5c09eba1 --json /tmp/eigenlayer-fuzz.json |
| iterations >= 5000 or justified lower bound | done | total=85000 pass=85000 soft=0 fail=0 panicked=0 skipped=0 seed=0x5c09eba1 iterations=5000 |
...
| representative pass/error corpus entries committed or justified | done | strict corpus PASS: 28/28 matched; semantic expect_body 26/26 pass entries pinned |
...
| Etherscan txlist pull executed adapter-blind by P0 cover addresses | done | crates/integration-tests/onboarding/eigenlayer/etherscan-mainnet-txlist-summary.json; Etherscan V2 txlist: api_calls=4 target_addresses=4 raw_txs_seen=30008 unique_selectors=27; defer processClaim seen; some low-volume cover selectors absent in latest 10k page |
| external tx pull target address count is nonzero and recorded | done | Etherscan V2 txlist: api_calls=4 target_addresses=4 raw_txs_seen=30008 unique_selectors=27; defer processClaim seen; some low-volume cover selectors absent in latest 10k page |
| Etherscan `api_calls_used` recorded | done | crates/integration-tests/onboarding/eigenlayer/etherscan-mainnet-txlist-summary.json records api_calls_used |
| Etherscan `raw_txs_seen` recorded | done | Etherscan V2 txlist: api_calls=4 target_addresses=4 raw_txs_seen=30008 unique_selectors=27; defer processClaim seen; some low-volume cover selectors absent in latest 10k page |
| Etherscan `unique_selectors_seen` recorded | done | Etherscan V2 txlist: api_calls=4 target_addresses=4 raw_txs_seen=30008 unique_selectors=27; defer processClaim seen; some low-volume cover selectors absent in latest 10k page |
| Etherscan real tx coverage per COVER selector recorded | done | crates/integration-tests/onboarding/eigenlayer/etherscan-mainnet-txlist-summary.json records covered_selectors_seen/missing per target address |
...
| unknown to-addresses with known protocol selectors bucketed as P0/P2 hard gaps | done | Unknown selector bucket recorded: 13/4142 = 0.3139% Dune 30d; no unknown to-address in representative target set |
...
| Dune calibration/query executed with partition WHERE or explicitly blocked | done | crates/integration-tests/onboarding/eigenlayer/dune-mainnet-coverage-share-30d.json; partition WHERE block_date >= current_date - interval 30 day |
...
| Dune rows returned / selected tx hashes recorded | done | Dune query 7638869 rows=70 cost=4.054 credits; cover 3184/4142 = 76.8711%, defer 945/4142 = 22.8151%, unknown 13/4142 = 0.3139% |
| representative real-tx corpus/golden entries committed or justified | done | strict corpus PASS: 28/28 matched; semantic expect_body 26/26 pass entries pinned |
| protocol-filtered corpus replay executed with semantic pin gate: `v3-harness corpus --filter <protocol> --require-expect-body` | done | cargo run -p policy-engine-integration-tests --bin v3-harness -- corpus --filter eigenlayer --require-expect-body PASS; strict corpus PASS: 28/28 matched; semantic expect_body 26/26 pass entries pinned |

## P3 Develop Evidence

| required evidence | status | artifact / exact command / summary |
|---|---|---|
| all P2 hard/soft/misdecoded/unknown_protocol_address/excluded gaps bucketed | done | Gaps bucketed: deferred rewards.processClaim share 22.8151%, unknown selector share 0.3139% |
...
| P2 rerun after fixes recorded | done | total=85000 pass=85000 soft=0 fail=0 panicked=0 skipped=0 seed=0x5c09eba1 iterations=5000; strict corpus PASS: 28/28 matched; semantic expect_body 26/26 pass entries pinned |
...
| remaining gaps have explicit defer/blocker disposition | done | Remaining user-facing DEFER: rewards.processClaim = measured 22.8151%; completion claim limited to covered restaking/permission semantics |
...
| v3-harness coverage/fuzz/corpus outputs recorded | done | total=85000 pass=85000 soft=0 fail=0 panicked=0 skipped=0 seed=0x5c09eba1 iterations=5000; strict corpus PASS: 28/28 matched; semantic expect_body 26/26 pass entries pinned |
| protocol-filtered strict corpus output recorded: `v3-harness corpus --filter <protocol> --require-expect-body` | done | strict corpus PASS: 28/28 matched; semantic expect_body 26/26 pass entries pinned |
...
| remaining WARNs/deferred selectors/actions listed with reason | done | WARN/defer: EigenLayer rewards.processClaim deferred with measured 22.8151% share |
