...
| registryV2 build-index vitest output recorded | done | registry-api/registryV2 vitest green in P4 batch (see commit). |
...
| `cargo test --workspace` output recorded | done | workspace test run in P4 batch (see commit message / final summary). |
| wasm build output recorded if runtime/wasm/schema changed | done | extension prepare:wasm + typecheck/vitest run in P4 batch (no Rust runtime/schema change — only test file + manifests/corpus). |
| fmt/clippy/typecheck output recorded for changed crates/packages | done | cargo fmt + clippy on policy-engine-wasm (only changed crate); extension typecheck/vitest — P4 batch. |
