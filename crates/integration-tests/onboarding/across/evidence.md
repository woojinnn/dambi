...
| covered real-usage coverage-share — **volume-weighted protocol-level**: Σ covered top-level tx / Σ all top-level tx across every user-facing entry (NOT per-contract selector-share) (H2), wrappers counted by child resolution-rate (H3) | **CORRECTED (Dune 7659668, top-level tx.to of ALL Across mainnet deposits, 30d)**: SpokePool direct (covered) = 12,386 EOA (~28%); +Periphery 2,981 (~34%); **aggregator-routed (tx.to≠Across) = 29,425 (~66%)**. Covered = SpokePool direct depositV3+deposit = ~100% of direct-SpokePool / **~28% of ALL Across mainnet deposit-signing**. ⚠️ earlier '~80%' counted direct-vs-Periphery only (excl aggregators) — the morpho top-level-vs-internal trap; corrected here. |
| user-facing DEFERs, each with its 1st-party usage-share (%/count) | **(1) aggregator-routed Across = 29,425 EOA / ~66% (Dune 7659681: Li.Fi 0x1231de… 9,372 · 0x6946… 3,861 · 0x0439… 3,492 · Socket/Bungee 0x3a23f9… 3,464 · long tail + ERC-4337) — tx.to=aggregator, Across leg internal → needs AGGREGATOR onboarding, structurally NOT catchable by Across manifests.** (2) SpokePoolPeriphery 9 mutating fns (depositNative/swapAndBridge + …WithPermit/Permit2/Authorization + multicall), ~2,981 EOA (7659341/7659668). (3) SpokePool deposit variants each 0 tx/30d (7659328). (4) multichain. |
| direct factory-child calls | not applicable — SpokePool is a singleton proxy. |
| final claim label (MUST NOT over-claim the measured coverage-share above) | "Across **SpokePool DIRECT** deposit (depositV3+deposit), mainnet — ~100% of direct-SpokePool but **only ~28% of ALL Across mainnet deposit-signing** (Dune 7659668). Deferred: aggregator-routed ~66% (Li.Fi/Socket/…), Periphery ~6%, multichain. Anchor slice, NOT full Across." |
...
| SCOPE ORACLE — covered-surface real-usage coverage-share measured on the P0 universe (1st-party Etherscan/Dune: % of recent txs the covered set decodes), **volume-weighted protocol-level (Σ covered top-level tx / Σ all top-level tx across every user-facing entry, NOT per-contract selector-share) (H2)** and **every wrapper/router selector counted by child resolution-rate, not manifest-presence (H3)**, with each user-facing DEFER's usage-share recorded; completion label must not over-claim it | done | H2: depositV3+deposit = 12,462 signing-EOA = ~100% of SpokePool deposit-signing (Dune 7659328); ~80% of Across mainnet deposit-signing incl Periphery. H3 n/a (flat single_emit, not wrapper). **top-level vs internal (Dune 7659668): SpokePool direct = ~28% of ALL Across mainnet deposit-signing EOA, +Periphery ~34%, aggregator-routed (tx.to≠Across) ~66% (29,425 EOA)**. DEFER usage-share: aggregator-routed 29,425 EOA (Li.Fi 9,372/Socket 3,464/…, 7659681); SpokePoolPeriphery ~2,981 (7659341); deposit variants 0 tx; multichain. Completion label corrected to ~28% direct (not ~80%). |
...
| final completion label recorded without overclaiming wallet-facing/full-universe/multichain scope | done | Across SpokePool DIRECT deposit (depositV3+deposit), mainnet — ~100% of direct-SpokePool but **~28% of ALL Across mainnet deposit-signing** (Dune 7659668). Deferred (measured): aggregator-routed ~66% (Li.Fi/Socket/…, 7659681 — needs aggregator onboarding), SpokePoolPeriphery ~6% (7659341), multichain. Anchor slice, NOT full Across. |
...
**Claim:** Across **SpokePool DIRECT deposit** (depositV3 + deposit) onboarded, Ethereum mainnet — new `bridge` ActionBody domain (axis-1). Covers ~100% of direct-SpokePool deposit-signing but **~28% of ALL Across mainnet deposit-signing** (Dune 7659668). Honest deferrals (measured): **aggregator-routed ~66%** (Li.Fi/Socket/…, needs aggregator onboarding — structurally not catchable by Across manifests since ScopeBall routes by tx.to), SpokePoolPeriphery ~6%, multichain. All P0–P4 rows `done`; `--phase all` 71 rows 0 blocked. NOT pushed/merged.
