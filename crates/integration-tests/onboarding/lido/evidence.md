...
| remaining WARNs/deferred selectors/actions listed with reason | done | COVERED (L1): bare-ETH selectorless stake → 0x00000000 native-transfer sentinel (was orchestrator warn-closed). DEFER: multichain L2 wstETH; enrichment-RPC runtime materialization (framework dormant). ACCEPT: `*WithPermit` embedded grant value not surfaced (bounded, self-spender; L2 planned in LIMITATIONS.md). check:surface WARN = 21 UNGATED non-lido contracts. check:tokens WARN = 1338 registry-wide unresolved-underlying refs (pre-existing, none lido). |
