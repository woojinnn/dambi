...
| `cargo test --workspace` output recorded | done | Completed exit 0; `cargo test --workspace` = **1381 passed / 0 failed** (post-L1/L2/L3; includes the L2 `request_withdrawal_with_permit_conforms` conformance test). Crate consuming the lido corpus + goldens: `policy-engine-integration-tests` `v3_decode_harness` = 63 tests incl. **7 reasoned-from-calldata lido field goldens** (L3 added unwrap/claim/requestWstETH) + the 13/13 `--require-expect-body` corpus replay. |
