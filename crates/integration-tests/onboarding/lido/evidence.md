...
| representative real-tx corpus/golden entries committed or justified | done | 11-entry real-mainnet corpus (`data/golden/v3-decode/lido/corpus.json`): 9 one-per-cover-intent + 2 bare-ETH stakes (ETH→stETH fallback, ETH→wstETH receive; L1) + 4 hand goldens in `v3_decode_harness.rs` (submit amount/referral, requestWithdrawals owner/token, wrap amount, wrap live-input wiring). All field-level `expect_body`-pinned. |
| protocol-filtered corpus replay executed with semantic pin gate: `v3-harness corpus --filter <protocol> --require-expect-body` | done | `cargo run --bin v3-harness -- corpus --filter lido --require-expect-body` → corpus 11/11 matched; semantic expect_body 11/11 pass entries pinned; exit 0. |
...
| v3-harness coverage/fuzz/corpus outputs recorded | done | coverage: lido 14 single_emit (12 + 2 native-transfer sentinel). fuzz: 60000 pass / 0 fail (the 2 sentinel manifests are validate/fuzz-skipped by design — empty-calldata, not generic ABI fuzz; they are corpus-pinned instead). corpus: 11/11 matched (incl. 2 bare-ETH stakes). The `0x00000000` native-transfer sentinel is now COVERED for Lido (no longer deferred). |
| protocol-filtered strict corpus output recorded: `v3-harness corpus --filter <protocol> --require-expect-body` | done | `cargo run --bin v3-harness -- corpus --filter lido --require-expect-body` → 11/11 matched, semantic expect_body 11/11 pinned, exit 0. |
...
| remaining WARNs/deferred selectors/actions listed with reason | done | COVERED (L1): bare-ETH selectorless stake → 0x00000000 native-transfer sentinel (was orchestrator warn-closed). DEFER: multichain L2 wstETH; enrichment-RPC runtime materialization (framework dormant). COVERED (L2): `*WithPermit` embedded grant value/deadline now surfaced as `embedded_permit`. check:surface WARN = 21 UNGATED non-lido contracts. check:tokens WARN = 1338 registry-wide unresolved-underlying refs (pre-existing, none lido). |
