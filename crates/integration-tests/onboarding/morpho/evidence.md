...
| representative pass/error corpus entries committed or justified | done | Updated `crates/integration-tests/data/golden/v3-decode/morpho/corpus.json` so all 17 pass entries now have `expect_body`; two excluded-selector error entries remain expected error with no expect flip. |

## P2 Real-Tx Evidence

| required evidence | status | artifact / exact command / summary |
|---|---|---|
| Etherscan MCP/API availability checked | blocked | `test -n "$ETHERSCAN_API_KEY"` exited 1 and `test -f crates/integration-tests/.env` exited 1. No Etherscan MCP credentials were available in this session. |
| Etherscan txlist pull executed adapter-blind by P0 cover addresses | blocked | Not executed: missing Etherscan key/MCP blocks fresh adapter-blind 10k tx sweep for cover addresses `1/0xbbbbbbbbbb9cc5e90e3b3af64bdaf62c37eeffcb` and `8453/0xbbbbbbbbbb9cc5e90e3b3af64bdaf62c37eeffcb`. |
| external tx pull target address count is nonzero and recorded | done | Target count for pending Etherscan sweep is 2 scoped cover addresses. Existing committed corpus already contains 14 mainnet real tx entries plus 3 typed/hand fixtures and 2 excluded-selector errors. |
| Etherscan `api_calls_used` recorded | blocked | `api_calls_used=0` for this run because Etherscan key/MCP unavailable. |
| Etherscan `raw_txs_seen` recorded | blocked | `raw_txs_seen=0` for this run because Etherscan key/MCP unavailable. |
| Etherscan `unique_selectors_seen` recorded | blocked | `unique_selectors_seen=0` for this run because Etherscan key/MCP unavailable. |
| Etherscan real tx coverage per COVER selector recorded | blocked | Fresh Etherscan coverage not measured. Existing corpus covers all 7 on-chain COVER selectors on Ethereum with real txs and Base supply by hand fixture; fresh 10k adapter-blind sweep remains blocked. |
...
| unknown to-addresses with known protocol selectors bucketed as P0/P2 hard gaps | blocked | No fresh Etherscan txlist was available, so unknown to-address hard-gap bucketing could not be performed for the 47 official-docs multi-chain deployments. |
...
| Dune calibration/query executed with partition WHERE or explicitly blocked | done | Created and executed temp query `https://dune.com/queries/7630558` on free engine with `call_block_date >= current_date - interval '7' day`, chain filter `ethereum, base`, and 7 covered call tables from `morpho_blue_multichain`. |
| Dune `executionCostCredits` / usage delta recorded | done | Execution `01KT1WB5102VX8J76P5735YTHK`, engine `free`, `executionCostCredits=0.23`, usage delta `36.68 -> 36.91`. |
| Dune rows returned / selected tx hashes recorded | done | 14 rows returned. Sample hashes included Base borrow `0x0000fdf4846c3e12821fc9ab8d73d8c6fd05448465db6a7135da6e9f4ecb9580`, Base supply `0x0000d7f14a15ea0a5a4c89b50cfad114328b2c2b7a85a8616e8df8c791dae39d`, Ethereum supply `0x00006435a88ed4df70b69d11ad138c7b913f10d74dd08035d762fb2ca0ed54e0`, Ethereum setAuthorization `0x01e23cc36e6922544fcf95b5c65cb60417c33b434747c8092d25c192e936878b`. |
| representative real-tx corpus/golden entries committed or justified | done | Existing Morpho corpus has 14 Ethereum real tx entries from prior Etherscan v2 txlist pull and is now semantically pinned with `expect_body`. New representative import from fresh Etherscan is blocked by missing key/MCP. |
| protocol-filtered corpus replay executed with semantic pin gate: `v3-harness corpus --filter <protocol> --require-expect-body` | done | PASS: `cargo run -p policy-engine-integration-tests --bin v3-harness -- corpus --filter morpho --require-expect-body --json /tmp/morpho-corpus-pinned.json` => `corpus: 19/19 matched`, `semantic expect_body: 17/17 pass entries pinned`. |

## P3 Develop Evidence

| required evidence | status | artifact / exact command / summary |
|---|---|---|
| all P2 hard/soft/misdecoded/unknown_protocol_address/excluded gaps bucketed | done | No decode hard/soft/misdecode gaps after pinned corpus and fuzz. Remaining gaps are blocked_external_data for Etherscan sweep, blocked_multichain_surface for official-docs deployments beyond Ethereum/Base, and blocked_claude_code for second-opinion execution. |
| each fix tied to a gap id, selector, tx hash, or synthetic seed | done | Fix `semantic-pin-morpho-corpus` tied to all 17 pass corpus entries; fix `i0-scoped-morpho-deployments` tied to current Ethereum/Base Morpho Blue surface; fuzz seed `42` verified no synthetic decode gaps. |
| manifest/decoder/Tier3/harness change list recorded | done | No manifest, decoder, Tier3, or harness source changes. Changed files are corpus semantic assertions, scoped Morpho `_deployments.json`, and this evidence file. |
| P2 rerun after fixes recorded | done | Reran strict corpus after adding `expect_body`: 19/19 matched and 17/17 pass entries pinned. Reran fuzz after current full index: 80000/80000 pass. |
...
| remaining gaps have explicit defer/blocker disposition | done | Remaining blockers are listed in the Blockers table: Etherscan key/MCP absent, official-docs multi-chain Morpho expansion beyond current Ethereum/Base scope, and Claude Code local CLI failure. |
...
| v3-harness coverage/fuzz/corpus outputs recorded | done | Coverage PASS summary `callkeys=1550`, `typed_data_keys=82`, `install_failures=0`; fuzz PASS `total=80000 pass=80000`; corpus strict PASS `19/19 matched`, `17/17 pass entries pinned`. |
| protocol-filtered strict corpus output recorded: `v3-harness corpus --filter <protocol> --require-expect-body` | done | PASS command: `cargo run -p policy-engine-integration-tests --bin v3-harness -- corpus --filter morpho --require-expect-body --json /tmp/morpho-corpus-pinned.json`; output `semantic expect_body: 17/17 pass entries pinned`. |
...
| wasm build output recorded if runtime/wasm/schema changed | done | Not applicable for this PR: no runtime, wasm, schema, manifest, decoder, or Rust source changed. `cargo test --workspace` included `policy_engine_wasm` unit and integration tests passing. |
| fmt/clippy/typecheck output recorded for changed crates/packages | done | PASS: `cargo fmt --all -- --check`; PASS: `cargo clippy -p policy-engine-integration-tests --all-targets -- -D warnings`; PASS: `cd registryV2; npm run typecheck`. |
| exact staged files and commit hash recorded | done | Commit `60b72e5c` staged exactly: `registryV2/surface/morpho/_deployments.json`, `crates/integration-tests/data/golden/v3-decode/morpho/corpus.json`, `crates/integration-tests/onboarding/morpho/evidence.md`. |
| remaining WARNs/deferred selectors/actions listed with reason | done | Remaining WARNs are unrelated global surface warnings for aave and compound-v3 missing `_deployments.json`, plus 21 ungated protocol contracts across aave, hyperliquid, layerzero, standard. Morpho-specific deferred scope: official-docs Morpho Blue deployments beyond Ethereum/Base and fresh Etherscan 10k sweep. |
...
|---|---|---|
| Etherscan key/MCP unavailable, so fresh adapter-blind 10k real-tx sweep and fresh representative raw-tx import are blocked. | `test -n "$ETHERSCAN_API_KEY"` exit 1; `test -f crates/integration-tests/.env` exit 1; no Etherscan MCP credential present. | Provide `ETHERSCAN_API_KEY` or Etherscan MCP, then sweep chain 1 and 8453 Morpho singleton cover addresses and import representative txs. |
