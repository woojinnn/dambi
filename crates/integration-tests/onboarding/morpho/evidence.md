...
| representative pass/error corpus entries committed or justified | done | Updated `crates/integration-tests/data/golden/v3-decode/morpho/corpus.json` so all 24 pass entries now have `expect_body`: existing Ethereum/Base fixtures plus 7 fresh Etherscan V2 Ethereum representatives from the 2026-06-02 adapter-blind sweep. Two excluded-selector error entries remain expected error with no expect flip. |
...
| Etherscan real tx coverage per COVER selector recorded | done | `etherscan-cover-tx-summary.json` records 7/7 onchain COVER selectors observed on Ethereum: supply, withdraw, borrow, repay, supplyCollateral, withdrawCollateral, and setAuthorization. Base direct top-level 7/7 selector evidence is recorded in `dune-direct-base-summary.json`. |
...
| Dune `executionCostCredits` / usage delta recorded | done | Calibration `executionCostCredits=0.23`; Base direct top-level sweep `executionCostCredits=5.24`. |
...
| representative real-tx corpus/golden entries committed or justified | done | Existing Morpho corpus had 14 Ethereum real tx entries. This pass imported 7 additional fresh Etherscan V2 Ethereum representatives, one per onchain COVER selector, with semantic `expect_body` domain/action pins. Base direct real tx hashes are recorded in Dune evidence but not imported because Etherscan Base txlist is unavailable on the free plan. |
| protocol-filtered corpus replay executed with semantic pin gate: `v3-harness corpus --filter <protocol> --require-expect-body` | done | PASS after fresh real-tx import: `cargo run -p policy-engine-integration-tests --bin v3-harness -- corpus --filter morpho --require-expect-body` => `corpus: 26/26 matched`, `semantic expect_body: 24/24 pass entries pinned`. |

## P3 Develop Evidence

| required evidence | status | artifact / exact command / summary |
|---|---|---|
| all P2 hard/soft/misdecoded/unknown_protocol_address/excluded gaps bucketed | done | No decode hard/soft/misdecode gaps after pinned corpus and fuzz. Fresh Etherscan is closed for Ethereum and narrowed to an Etherscan free-plan limitation on Base, which is backfilled by Dune direct top-level rows. Remaining non-real-tx gaps are blocked_multichain_surface for official-docs deployments beyond Ethereum/Base and blocked_claude_code for second-opinion execution. |
| each fix tied to a gap id, selector, tx hash, or synthetic seed | done | Fix `semantic-pin-morpho-corpus` tied to all 24 pass corpus entries; fix `i0-scoped-morpho-deployments` tied to current Ethereum/Base Morpho Blue surface; fresh real-tx import tied to the 7 selected Etherscan hashes in `etherscan-cover-tx-summary.json`; Base direct evidence tied to query `7633652`; fuzz seed `42` verified no synthetic decode gaps. |
...
| P2 rerun after fixes recorded | done | Reran strict corpus after fresh Etherscan import: 26/26 matched and 24/24 pass entries pinned. Reran fuzz after current full index: 80000/80000 pass. |
...
| remaining gaps have explicit defer/blocker disposition | done | Remaining blockers are listed in the Blockers table: Base Etherscan free-plan txlist unavailable but Dune direct evidence recorded, official-docs multi-chain Morpho expansion beyond current Ethereum/Base scope, and Claude Code local CLI failure. |
...
| v3-harness coverage/fuzz/corpus outputs recorded | done | Coverage PASS summary `callkeys=1550`, `typed_data_keys=82`, `install_failures=0`; fuzz PASS `total=80000 pass=80000`; corpus strict PASS after fresh real-tx import `26/26 matched`, `24/24 pass entries pinned`. |
| protocol-filtered strict corpus output recorded: `v3-harness corpus --filter <protocol> --require-expect-body` | done | PASS command after fresh real-tx import: `cargo run -p policy-engine-integration-tests --bin v3-harness -- corpus --filter morpho --require-expect-body`; output `corpus: 26/26 matched`, `semantic expect_body: 24/24 pass entries pinned`. |
...
|---|---|---|
| Base Etherscan txlist unavailable on free API plan. | Etherscan V2 accepted the key and returned Ethereum rows, but chain 8453 returned `Free API access is not supported for this chain`. | Committed `dune-direct-base-summary.json` with Base direct top-level singleton evidence for all 7 onchain COVER selectors. If explorer-native Base txlist closure is mandatory, use a paid Etherscan plan or chain-specific explorer source. |
