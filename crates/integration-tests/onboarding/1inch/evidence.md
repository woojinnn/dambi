...
|---|---|---|
| build-index vitest not run in this onboarding worktree | browser-extension Yarn 4 / WASM toolchain not provisioned (same as other onboarding worktrees) | non-fatal — build correctness is covered by `npm run build` (validates every manifest + token) + `check:manifest` (1486 OK) + `check:surface` (PASS). Rerun in a provisioned env: `cd browser-extension && yarn && yarn vitest run --root ../registryV2 scripts/__tests__/build-index.test.ts`. (The round-1 `executor`-slot blocker is RESOLVED — see Follow-up Round 3.) |
