...
| covered real-usage coverage-share — **volume-weighted protocol-level**: Σ covered top-level tx / Σ all top-level tx across every user-facing entry (NOT per-contract selector-share) (H2), wrappers counted by child resolution-rate (H3) | TWO numbers (don't conflate): **routing = 100.0%** (all 51 selectors with ≥1 tx/30d covered → every diamond tx routes to a Li.Fi decoder). **effective FULL-decode = 92.2%** of the 10k window (swap-only 4,031 + bridge-leg 5,191). Includes non-EVM (Solana/Bitcoin) via `BridgeRecipient::Raw` from facet bytes32 for the 7 bytes32-receiver facets (AcrossV4/Mayan/NEARIntents/GasZip/AllBridge/Glacis/PolymerCCTP). **7.8% warn-close** = non-EVM via bytes/string-receiver facets (Chainflip/Squid/Eco/Symbiosis) + Sui-class + unknown chain ids. Li.Fi IS top-level. Count-weighted per H2. (**Li.Fi→Across specifically = 99.1%** decode, AcrossV4 EVM+Solana.) |
...
| final claim label (MUST NOT over-claim the measured coverage-share above) | "Li.Fi LiFiDiamond, Ethereum mainnet — routing 100% of 30d/10k top-level function-tx → a Li.Fi decoder; **~92% fully decode** (swap + EVM-destination bridge incl. source-swap legs + non-EVM Solana/Bitcoin via `BridgeRecipient::Raw` bytes32 for the 7 bytes32-receiver facets); **~8% warn-close** = non-EVM via bytes/string-receiver facets (Chainflip/Squid/Eco/Symbiosis) + Sui-class + unknown chain ids. **Li.Fi→Across = ~99%**. Deferred: facet dst_token/output/exclusiveRelayer enrichment, `*Packed` variants, multichain. NOT full-universe, NOT multichain." |
...
| representative pass/error corpus entries committed or justified | done | `crates/integration-tests/data/golden/v3-decode/lifi/corpus.json` — 18 real txs, all `expect:pass`: 16 EVM-dest + 2 non-EVM (AcrossV4→Solana, Mayan→Solana) pinning `dst_recipient = Raw{bytes32}` (the real Solana recipient) + `dst_chain = solana:…`. 200 `expect_body` pins from independent `cast decode-calldata`. |
...
| Etherscan real tx coverage per COVER selector recorded | done | All 51 cover selectors observed in the 10k window; routing coverage = 10,000/10,000 = 100.00% route to a covered selector. **Bulk decode**: all 10,000 real txs imported (`import-etherscan`) + run through the harness decoder → 9,222 pass / 778 warn-close = 92.2% effective full-decode (per-tx actual, not analytical). |
...
| representative real-tx corpus/golden entries committed or justified | done | 18-tx corpus committed: 16 EVM facets/4 decode shapes + 2 non-EVM (Solana via AcrossV4/Mayan, Raw recipient), all pass. |
| protocol-filtered corpus replay executed with semantic pin gate: `v3-harness corpus --filter <protocol> --require-expect-body` | done | `v3-harness corpus --filter lifi --require-expect-body` → **18/18 matched**, semantic expect_body **18/18 pinned** (200 assertions). |
| SCOPE ORACLE — covered-surface real-usage coverage-share measured on the P0 universe (1st-party Etherscan/Dune: % of recent txs the covered set decodes), **volume-weighted protocol-level (Σ covered top-level tx / Σ all top-level tx across every user-facing entry, NOT per-contract selector-share) (H2)** and **every wrapper/router selector counted by child resolution-rate, not manifest-presence (H3)**, with each user-facing DEFER's usage-share recorded; completion label must not over-claim it | done | **Routing coverage = 100.00%** (10,000/10,000 diamond txs → a covered selector). **Effective FULL-decode = 92.2%** (swap-only 4,031 + bridge-leg 5,191 = 9,222/10,000), facet-aware. Non-EVM destinations (Solana/Bitcoin) decode via **`BridgeRecipient::Raw`** from the facet bytes32 (`bridge_recipient` $fn keys on Li.Fi's `0x11f1..f1` non-EVM sentinel) for the 7 bytes32-receiver facets (AcrossV4/Mayan/NEARIntents/GasZip/AllBridge/Glacis/PolymerCCTP) — verified on tx 0x011f… (Raw bytes32 = AcrossV4Data.receiverAddress 0x0f64…0d7f). **7.8% (778) warn-close** = non-EVM via bytes/string-receiver facets (Chainflip/Squid/Eco/Symbiosis, receiver is variable `bytes`/`string` not bytes32) + Sui-class (9270000000000000) + unknown chain ids (1337/4326). Bridge-leg decode = 5,191/5,969 = 87.0%. **Li.Fi→Across = 1,367/1,380 = 99.1%** (AcrossV4 EVM + Solana via Raw). **(H3)** N/A — composite Multicall is in-place. **DEFER usage-share:** non-EVM-via-bytes/string + Sui + unknown 7.8%; 48 inactive selectors 0 tx/30d. **CONFIRMED by bulk decode (not just analytical):** `import-etherscan /tmp/lifi_txlist.json → corpus --root … --filter lifi` ran ALL 10,000 real Etherscan txs through the actual harness decoder → **9,222/10,000 pass (92.2%)**, 778 soft(build_action_body_failed)=warn-close — matches the field-level projection exactly. ⚠️ honesty trail: an interim draft mapped Solana but kept the EVM recipient → would have emitted the bogus sentinel (caught in review, G4); then implemented Raw bytes32 recipient (G5). Li.Fi is itself top-level. |
...
| each fix tied to a gap id, selector, tx hash, or synthetic seed | done | **G1** → 0x782621d8 / 0xaf62c7d6 (fuzz seed 0x958642a3635015db) → flattened field-name args when `len(inputs)==1`. **G2** → +5 EVM dest chains (2020/25/30/143/98866) to the value-map. **G4** → value-map made EVM-only (interim safe fix: removed Solana/Tron → non-EVM warn-close, no bogus recipient). **G5 (proper fix)** → new `bridge_recipient` $fn (keys on the `0x11f1..f1` sentinel → `BridgeRecipient::Raw{bytes32}` from the facet's non-EVM receiver, else Evm) + re-added Solana/Bitcoin CAIP-2 to the value-map of the **7 bytes32-receiver facets** (AcrossV4 idx0 / Mayan idx0 / NEARIntents idx0 / GasZip idx0 / AllBridge idx0 / Glacis idx0 / PolymerCCTP idx2). Now non-EVM destinations DECODE with the real recipient — verified on tx 0x011f… (`dst_recipient = Raw{0x0f64…0d7f}`, dst_chain=`solana:5eykt4…`). |
| manifest/decoder/Tier3/harness change list recorded | done | decoder: `composite_emit` strategy (declarative_exports.rs), `token_key_or_native_zero` + `bridge_recipient` $fns (builtin_fn.rs + fn_whitelist.json + unit tests). manifests: 51 regenerated (G1 single-tuple-flatten + G2 EVM-chain adds + G5 bridge_recipient/Raw for 7 bytes32 facets + CAIP2_FULL with Solana/Bitcoin). corpus: 18 (2 Solana Raw-recipient pins). No harness/Tier3 change. |
| P2 rerun after fixes recorded | done | post-fix: fuzz 150,000 → HARD_fail 0/panicked 0; corpus **18/18** (incl. 2 Solana non-EVM Raw-recipient pins) + 200 expect_body assertions; routing 100%; **effective full-decode 92.2%** (honest, facet-aware; non-EVM via Raw bytes32). The earlier 92.4% was an overclaim (Solana decoded with the bogus sentinel); after G4 (warn-close) → 77.3%, then G5 (Raw recipient) → 92.2% correctly. Li.Fi→Across 99.1%. |
...
| remaining gaps have explicit defer/blocker disposition | done | **Deferred (measured, now only 7.8%):** non-EVM destinations via the **bytes/string-receiver facets** (Chainflip/Squid/Eco/Symbiosis — receiver is variable `bytes`/`string`, doesn't fit `Raw{bytes32}` cleanly) + Sui-class chain id (9270000000000000) + unknown chain ids (1337/4326). EVM + Solana/Bitcoin destinations of the 7 bytes32-receiver facets now FULLY decode (G5). Also deferred: 48 inactive selectors (0 tx/30d); facet `dst_token`/`output_amount`/`exclusiveRelayer` enrichment; `*Packed` variants. Source-swap leg IS decoded (composite_emit). No blockers. |
...
| v3-harness coverage/fuzz/corpus outputs recorded | done | coverage: `callkeys=2275 ... unique_bundles=1064 install_failures=0` (composite_emit bundles install clean). fuzz: 150,000 / HARD_fail 0 / panicked 0. corpus: 18/18 matched. |
| protocol-filtered strict corpus output recorded: `v3-harness corpus --filter <protocol> --require-expect-body` | done | `v3-harness corpus --filter lifi --require-expect-body` → **18/18 matched** · semantic expect_body 18/18 pinned (200 assertions, incl. 2 non-EVM Raw-recipient). |
...
| remaining WARNs/deferred selectors/actions listed with reason | done | **Deferred (7.8% of 10k):** non-EVM destinations via **bytes/string-receiver facets** (Chainflip/Squid/Eco/Symbiosis — receiver not bytes32) + Sui-class (9270000000000000) + unknown chain ids (1337/4326); 48 inactive bridge/swap selectors (0 tx/30d); facet-specific dst_token/output_amount/exclusiveRelayer enrichment; `*Packed` calldata variants; LiFiDEXAggregator periphery; multichain. **Pre-existing WARNs (not lifi):** check:tokens 1338 warns/0 errors; build-index "skipped 239 sourced duplicate callkey"; check:surface I0' aave/morpho stale-deployment notes. |
| final completion label recorded without overclaiming wallet-facing/full-universe/multichain scope | done | "Li.Fi LiFiDiamond, Ethereum mainnet — routing 100% of 30d/10k top-level function-tx → a Li.Fi decoder; **~92% fully decode** (swap + EVM-destination bridge incl. source-swap legs + non-EVM Solana/Bitcoin via Raw bytes32 recipient for the 7 bytes32-receiver facets); **~8% warn-close** = non-EVM via bytes/string-receiver facets (Chainflip/Squid/Eco/Symbiosis) + Sui-class + unknown chain ids. **Li.Fi→Across = ~99%**. Deferred: facet dst_token/output enrichment, `*Packed`, multichain. NOT full-universe / NOT multichain." |
