...
[입력: 새 프로토콜]
   ...
   │
   P4 LAND ── build-index → build-index vitest → check:manifest(emit.body shape) → check:surface → coverage/fuzz/corpus → cargo test --workspace 0 fail → fmt/clippy → commit
```
...
온보딩 "완료" 선언 전 7문 — 하나라도 No 면 미완. 0·4 가 build 강제(컨트랙트·함수 전수성), 1~3·5~6 은 산문 자가점검(enrichment/token-surface 는 build-gate 가 부분적):
