...
- 새 프로토콜의 진입점 함수에 대한 **어댑터(Tier 1 manifest) 전수 작성**, 필요 시 **generic 엔진 확장(Tier 2)** 과 **ActionBody 스키마 확장(Tier 3)**.
...
raw Tx { chain, to, selector, calldata, value }
  ...
  ▼  declarative_route_request_v3_json(input_json)       declarative_exports.rs:329  ★단일 오케스트레이터(~737)
     ...
     ▼
  Vec<ActionBody>   (serde tag="domain")                  policy-server/asset-model/action/src/lib.rs:149
```
...
| **ActionBody 스키마(Tier 3)** | `crates/policy-server/asset-model/action/src/**` | `ActionBody`(lib.rs:149) + domain sub-enum |
...
| **Tier 3 — ActionBody 스키마** | 디코드 결과 타입(=정규화 산출) | `crates/policy-server/asset-model/action/src/` | 가장 무거움 (Cedar/TS/oracle 동기화) |
...
> 이 표는 **요약**이다. 필수 evidence 항목의 SSOT·강제 = `ONBOARDING_EVIDENCE_TEMPLATE.md` + `check-onboarding-evidence`. "온보딩 완료" 의 정의 자체 = `PROTOCOL_AGNOSTIC_ONBOARDING_FRAMEWORK.md` §6 (SSOT). 충돌 시 그 둘이 우선.
...
필수 산출물:
1. **ActionBody schema** — `crates/policy-server/asset-model/action/src/<domain>/**`: protocol-specific 이름이 아니라 user intent 를 표현하는 domain/action/field.
2. **effect/reducer/view/sync touchpoint** — `crates/policy-server/asset-model/transition/src/effect/**`, `crates/policy-server/asset-model/action/src/view.rs`, 필요 시 `policy-server/sync/src/actions/walk/**`.
...
#### ActionBody 카탈로그 (현재 — `policy-server/asset-model/action/src/`, serde `tag="domain"`)
...
1. `crates/policy-server/asset-model/action/src/lending/flash_loan.rs` 새 파일 — action struct (`#[derive(Serialize, Deserialize, Tsify, ...)]`).
...
`DataSource::OnchainView { chain, contract, function, decoder_id }` 에는 **args 필드가 없다**(`policy-server/asset-model/state/src/live_field/source.rs`). 즉 `getPooledEthByShares(shares)` 처럼 **디코드된 calldata 값을 view 인자로 넘겨야 하는** live_field 는 manifest 의 `onchain_view` 만으로 불가능하다. 인자는 sync 시점에 **`crates/policy-server/sync/src/actions/args.rs` 의 `resolve_args(slot, action, state)`** 가 `ActionSlot` 별로 action 에서 추출해 인코딩한다 → **Tier 2 Rust 필수**. (인자 없는 view(`getTotalShares()`)나 derived 계산은 manifest-only 가능.)

| live_field 종류 | manifest-only? | 작업 |
|---|---|---|
| 인자 없는 view (`getTotalShares()`) / oracle_feed / derived_from | ✅ | manifest `live_inputs.source` 만 |
| **calldata 인자 필요한 view** (`getPooledEthByShares(shares)`) | ❌ | + Tier 2 `ActionSlot` + `args` arm |
...
|---|---|---|
| **A** reducer | `<Action>LiveInputs { <field>: LiveField<T> }` struct + action 에 `pub live_inputs` 필드 (non-optional, manifest 가 항상 emit) | `crates/policy-server/asset-model/action/src/<domain>/<action>.rs` |
| **B** sync (Tier 2) | ① `ActionSlot` variant ② `actions/walk/<domain>.rs` 의 `walk`(`push_if_stale(&li.<field>, slot)`) + `apply`(slot match→`set_field`) + mod dispatch arm ③ `args::resolve_args` arm — action 에서 인자 추출 후 `encode_u256`(U256) / `encode_address`(addr) (둘 다 `fetchers/decoder.rs` 에 **이미 존재**) | `crates/policy-server/sync/src/live/walker.rs` · `sync/src/actions/walk/<domain>.rs` · `crates/policy-server/sync/src/actions/args.rs` |
...
> `value_to_u256`/`value_to_decimal` = `actions/walk/mod.rs` 공유 헬퍼. struct skeleton(`lending_reserve_state_skeleton()` 류)·`lower_<struct>` 는 해당 domain 의 기존 helper 미러. **golden 의 `source.function` pin(아래)은 `T` 무관**(source 메타는 값 타입과 독립) — 어느 타입이든 같은 방식으로 wiring 검증.
...
> 이 매뉴얼을 **dogfood** 으로 검증하며 만든 실제 온보딩 기록. 모든 커맨드·출력은 실측(2026-05-30, branch `feat/registry-v2`). 결과물은 commit `760af8c` 로 영구 온보딩됨(manifest 4 + Tier 2 Rust + golden corpus + golden test).
> Morpho Blue 를 고른 이유: lending 이고 `LendingVenue::MorphoBlue` 가 schema 에 이미 존재(=Tier 3 불필요) — "manifest+test 90% 케이스"로 보였다. **그런데 `market_id` 가 keccak 해시라 manifest-only 로 안 됐다.** 이 갭이 이 예시의 핵심 — manifest→Tier 2 에스컬레이션과, 자동 오라클이 못 잡는 **silent MIS-DECODED** 를 코드로 실증한다.
...
**9.2a schema 확인** — `crates/policy-server/asset-model/action/src/lending/mod.rs`: `LendingVenue::MorphoBlue { chain, market_id: String }` + supply/withdraw/borrow/repay action 전부 존재 → **Tier 3 변경 0**. 단 `market_id` 가 **keccak 해시 String** 임을 발견 — placeholder 문법(index/slice 만, hash 불가)으로 못 만든다.
...
```
RUN #2 의 248 = supply/withdraw/borrow/repay. manifest 가 라우팅되기 시작했으나 `$derived.morpho_market_id` 가 미해소 → **hard error**(silent 아님 — 시스템이 거부). 이게 Tier 2 가 필요한 신호.

### 9.4 P3 — Develop (Tier 2 + golden + silent MIS-DECODED 실증)
**Tier 2** — `declarative_exports.rs` 에 `maybe_inject_morpho_market_id` + `compute_morpho_market_id`(= `maybe_inject_v4_pool_id` 미러; address×4 left-pad + uint256 lltv = 0xa0 버퍼 → `keccak256`). single_emit 의 `ctx.derived` 에 주입(decode 직후, `match strategy` 전).
```bash
$ cargo build --bin v3-harness                          # policy-engine-wasm 재컴파일
# RUN #3 — Tier 2 후 (corpus 는 아직 전부 expect:pass)
$ target/debug/v3-harness corpus --root /tmp/morpho
...
- **방법론이 실제로 돈다**: P0(1차출처)→P1(schema 확인+manifest)→P2(blind pull+3단계 corpus)→P3(Tier 2+golden)→P4(workspace 0 fail+surgical commit) 가 멈춤 없이 관통.
- **manifest-only 90% 가정은 깨질 수 있다**: keccak 파생 필드(market_id, pool_id 류)는 Tier 2 Rust 필수. 선례(`maybe_inject_v4_pool_id`)를 먼저 찾아 미러하면 bounded.
...
- **수치**: manifest 4 + Tier 2 helper 2 fn(~50 LOC) + golden 1 + corpus 13건. authoring 비용 ∝ selector 수(여기 6, 작성 4) — tx 수(400)와 무관(§2).
...
**B1 collateral (쉬움, schema 변경 0)**: supplyCollateral→`Supply{asset=$args.marketParams[1](collateralToken)}`, withdrawCollateral→`Withdraw{asset=marketParams[1], recipient=receiver}`. 기존 action 재사용 + market_id Tier 2 그대로. corpus 248→**352**.
...
- B `ActionSlot::LiquidStakingTransferSharesPooledEth` + `actions/walk/liquid_staking.rs`(walk/apply) + `args` arm(`encode_u256(t.shares)` — `encode_u256` 는 `fetchers/decoder.rs` 에 이미 있었음)
