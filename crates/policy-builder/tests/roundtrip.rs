...
fn token_address_membership_installs() {
    let rule = PolicyRule {
        ...
        predicates: vec![Predicate {
            field: "tokenOut.address".into(),
            op: "in".into(),
            value: PredicateValue::Multi(vec![
                "0xrugpull0000000000000000000000000000dead".into(),
            ]),
        }],
    };
    let text = compile(&rule, &swap::schema()).expect("compile");
...
fn bool_predicate_installs() {
    let rule = PolicyRule {
        id: "user/allowance-required".into(),
        action: "swap".into(),
        severity: Severity::Warn,
        reason: "allowance does not cover the input amount".into(),
        predicates: vec![Predicate {
            field: "allowancesCoverInputs".into(),
            op: "isFalse".into(),
