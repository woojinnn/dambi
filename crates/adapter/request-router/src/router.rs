...
mod tests {
    ...
    #[test]
    fn test_route_request_dispatches_call_for_pancake_universal_router() {
        // Real PancakeSwap UR mainnet traffic captured from a frontend:
        //   to:       0x65b382653f7C31bC0Af67f188122035461ec9C76 (Pancake UR)
        //   selector: 0x3593564c (executeWithDeadline)
        //   commands: [0x0b WRAP_ETH, 0x00 V3_SWAP_EXACT_IN,
        //              0x06 PAY_PORTION, 0x04 SWEEP]
        //   path:     WETH 0x...c756cc2 → USDC 0x...606eb48 (fee 0x0001f4 = 500)
        //
        // PR 6b registered Pancake UR alongside Uniswap UR. Before that, this
        // returned `RouterError::NoMatch` because MultiRouterCallAdapter's
        // match_keys() only knew about Uniswap UR addresses.
        //
        // Expected output after the merge step in PR 3:
        //   - WRAP_ETH + V3_SWAP_EXACT_IN(WETH→USDC) collapse to
        //     SwapAction(ETH → USDC).
        //   - PAY_PORTION and SWEEP are Ignored (settlement plumbing).
        //   - Final: 1 SwapAction envelope.
        let params = json!([{
            "from": "0x7ee04c7057Ad92B7DCC8E9bB26358C6b0a62822C",
            "to": "0x65b382653f7C31bC0Af67f188122035461ec9C76",
            "value": "0x1b48eb57e000",
            "data": ""
        }]);

        let envelopes = route("eth_sendTransaction", params, 1);

        // After PR 11 the simulator sees SWEEP as a TransferAction
        // (router→user), closing the asset-flow loop:
        //   msg.value(ETH) → router; WRAP burns ETH, mints WETH on router;
        //   SWAP burns router's WETH, mints USDC on router; SWEEP transfers
        //   router USDC to user. Net user delta: ETH -, USDC +.
        // → simulator collapses to a single Swap(ETH → USDC) envelope.
        assert_eq!(envelopes.len(), 1, "Pancake UR WRAP+V3_SWAP+SWEEP should collapse to 1 swap");
