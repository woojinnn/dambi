...
fn text_to_est_to_text_is_stable() {
    ...
    // EST -> text
    let back: Value =
        serde_json::from_str(&est_json_to_policy_text(serde_json::to_string(&est).unwrap()))
            .unwrap();
    ...
    let back2: Value =
        serde_json::from_str(&est_json_to_policy_text(serde_json::to_string(&est2).unwrap()))
            .unwrap();
    assert_eq!(
        text1,
...
fn malformed_text_reports_error_not_panic() {
    let out: Value =
        serde_json::from_str(&policy_text_to_est_json("permit(".to_string())).unwrap();
