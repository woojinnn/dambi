...
import { provisionWallets } from "../server-api/policy-store";
...
export function AddWalletModal({
  ...
  const mut = useMutation({
    ...
    onSuccess: (resp) => {
      void provisionWallets([resp.wallet_id.address])
        .then(() => qc.invalidateQueries({ queryKey: ["ps2-overview"] }))
        .catch((err) =>
          console.warn(
            "[Dambi] ps2 wallet provisioning failed after add:",
            err,
          ),
        );
        ...
        mut.data ? (
          ...
          <>
            ...
            <button
              ...
              disabled={mut.isPending || !addressOk || !labelOk}
            >
            ...
            <div className="chain-chip-row" aria-live="polite">
              {syncChainsQ.data?.chains.length ? (
                ...
                <span className="chain-chip muted">
                  {syncChainsQ.isLoading
                    ? t("loading")
                    : t("wallet.chainsServerDefault")}
                </span>
              )}
            </div>
