...
export function HomePage() {
  ...
  // Single /audit/verdicts?wallet=... per wallet, 24h window, limit 50.
  ...
  return (
    <>
      <Topbar
        here="Scopeball Home"
        subtitle={summaryQ.data ? `${summaryQ.data.wallet_count} wallets · ${summaryQ.data.policy_count} policies` : "…"}
        counts={countsQ.data}
      />
      <ContextBar
...
function TriageRow({ f }: { f: VerdictDto }) {
  ...
  const editorHref = f.policy?.id ? `/server/editor?policy=${f.policy.id}` : "/server/editor";
