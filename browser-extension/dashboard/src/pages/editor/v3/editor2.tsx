...
import { useOverview, pushToast } from "./shell";
import { navigate } from "./nav";
import { run, defUsageCount, packageDisplayOn, DRAG_DEF_MIME } from "./list";
import { catKey, catStyle, shortAddr } from "./icons";
import { PublishModal } from "./modals";
import { PolicyFormPane } from "./formpane";
import { E2PolicyPreview } from "./preview";
...
let e2Drag = null;
export function Editor2View({ onNewPolicy }) {
...
function E2PackageCard({ pkg, wallet, snap, members, address, highlighted, pinned, onToggle, onRename, onSaveDesc, onRemove, onPublish, onDrop, previewId, onPreviewBinding, onForkBinding, onOpenBinding }) {
  ...
  return (
    <div
      ref={rowRef}
      data-pkgid={pkg.id}
      className={`prow${drop ? " drop" : ""}${locked ? " uncat" : ""}${highlighted ? " lensed" : ""}${pinned ? " pinned" : ""}`}
...
function E2BindingItem({ binding: b, def, address, selected, readOnly, onPreview, onOpen, onFork }) {
  ...
  return (
    <div className={`pb-item${b.enabled ? "" : " off"}`} title="누르면 이 지갑 값 수정 (구조는 읽기 전용)" onClick={onOpen}>
