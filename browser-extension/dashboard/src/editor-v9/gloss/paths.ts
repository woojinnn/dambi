...
const BASE_GLOSS_ENTRIES: readonly GlossEntry[] = [
  ...
  { path: "context.venue.name", ko: "베뉴 이름", en: "Venue name",
    role: "ref", fieldKind: "primitive.String",
    desc: { ko: "베뉴 식별자 (Hyperliquid 정책은 \"hyperliquid\"로 매칭)", en: "Venue id string (HL policies match \"hyperliquid\")" } },
  ...
  { path: "context.amount", ko: "수량 (raw)", en: "Amount (raw)",
    ...
    desc: { ko: "토큰 단위 그대로의 거래량 (hex). 크기 비교(≥,≤)는 '수량 (nano)' 사용", en: "Token-unit amount (hex)" } },
  // nano-normalized amount siblings — the engine emits `<field>Nano` (= token
  // amount × 10^9) for each raw uint256 amount. These are Long, so they support
  // < ≤ > ≥ (Cedar has no ordering on the raw String amounts). Use for size caps.
  { path: "context.amountNano", ko: "수량 (nano)", en: "Amount (nano)",
    role: "numeric", fieldKind: "primitive.Long",
    desc: { ko: "거래량을 나노 단위로 (10^9 = 1토큰). 예: 0.05토큰 = 50000000", en: "Amount in nano (10^9 = 1 token)" },
    unit: { ko: "nano", en: "nano" } },
  { path: "context.amountInNano", ko: "입력 수량 (nano)", en: "Amount in (nano)",
    role: "numeric", fieldKind: "primitive.Long",
    desc: { ko: "스왑 입력량을 나노 단위로 (10^9 = 1토큰)", en: "Swap input in nano (10^9 = 1 token)" },
    unit: { ko: "nano", en: "nano" } },
  { path: "context.amountOutNano", ko: "출력 수량 (nano)", en: "Amount out (nano)",
    role: "numeric", fieldKind: "primitive.Long",
    desc: { ko: "스왑 출력량을 나노 단위로 (10^9 = 1토큰)", en: "Swap output in nano (10^9 = 1 token)" },
    unit: { ko: "nano", en: "nano" } },
  { path: "context.minAmountOutNano", ko: "최소 출력 수량 (nano)", en: "Min amount out (nano)",
    role: "numeric", fieldKind: "primitive.Long",
    desc: { ko: "최소 출력 토큰량을 나노 단위로 (10^9 = 1토큰)", en: "Min output in nano (10^9 = 1 token)" },
    unit: { ko: "nano", en: "nano" } },
  { path: "context.maxAmountInNano", ko: "최대 입력 수량 (nano)", en: "Max amount in (nano)",
    role: "numeric", fieldKind: "primitive.Long",
    desc: { ko: "최대 입력 토큰량을 나노 단위로 (10^9 = 1토큰)", en: "Max input in nano (10^9 = 1 token)" },
    unit: { ko: "nano", en: "nano" } },
  { path: "context.sellAmountNano", ko: "매도 수량 (nano)", en: "Sell amount (nano)",
    role: "numeric", fieldKind: "primitive.Long",
    desc: { ko: "매도 토큰량을 나노 단위로 (10^9 = 1토큰)", en: "Sell amount in nano (10^9 = 1 token)" },
    unit: { ko: "nano", en: "nano" } },
  { path: "context.buyMinNano", ko: "최소 매수량 (nano)", en: "Buy min (nano)",
    role: "numeric", fieldKind: "primitive.Long",
    desc: { ko: "최소 매수 토큰량을 나노 단위로 (10^9 = 1토큰)", en: "Buy min in nano (10^9 = 1 token)" },
    unit: { ko: "nano", en: "nano" } },
  ...
  { path: "context.price", ko: "주문 가격", en: "Order price",
    role: "numeric", fieldKind: "primitive.String",
    desc: { ko: "Hyperliquid 주문의 지정가 (소수 문자열, Cedar decimal 4자리 초과)", en: "Hyperliquid order limit price (decimal string; exceeds Cedar's 4-dp decimal)" } },
  { path: "context.leverage", ko: "유효 레버리지", en: "Effective leverage",
    role: "numeric", fieldKind: "primitive.Long",
    desc: { ko: "이 주문의 자산별 유효 레버리지 (호스트가 채움; 없을 수 있어 has로 가드)", en: "Per-asset effective leverage for this order (host-enriched; may be absent — guard with has)" },
    unit: { ko: "x", en: "x" } },
  ...
  { path: "context.positionEffect", ko: "포지션 효과", en: "Position effect",
    role: "enum", fieldKind: "primitive.String",
    desc: { ko: "HL 주문 의도: \"open\"(신규/추가) vs \"reduce\"(감소). side(매수/매도)와 별개 — 신규 숏만 막으려면 side==\"short\" && positionEffect==\"open\"", en: "HL order intent: \"open\" vs \"reduce\" (orthogonal to side); match side==\"short\" && positionEffect==\"open\" to block only new shorts" } },
  { path: "context.tif", ko: "주문 유효기간", en: "Time in force",
    role: "enum", fieldKind: "primitive.String",
    desc: { ko: "HL 주문 TIF: \"gtc\" | \"ioc\" | \"post_only\"", en: "HL order time-in-force: \"gtc\" | \"ioc\" | \"post_only\"" } },
