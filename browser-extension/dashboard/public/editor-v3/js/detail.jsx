...
function EditorDetailPageV2() {
  ...
  return (
    <>
      <Topbar here="Policy Editor" subtitle={policy ? policy.displayName : id || "…"} right={<a href="#/editor" className="ev2-back">← 목록</a>} />
      <div className="ev2-detail-body">
        {!policy && (
          <div className="ev2-empty">
            <div className="big">정책을 찾을 수 없습니다</div>
            <div className="sm"><code>{id}</code><br /><a href="#/editor">← 목록으로 돌아가기</a></div>
          </div>
        )}
        {policy && (
          <EditorBody
...
// consume seed once, but keep it across this id's lifetime (re-renders)
const _seedCache = {};
...
function EditorBody({ policy, storedDef, snap, bindingCtx, isNew, defaultScope, defaultWallet, replaceCtx, initialTab, onSaved, onDeleted }) {
  ...
  // 정책 소개(문서): 정의·범위·대상·사용 데이터. 빈 칸은 저장하지 않음.
  ...
  const doSave = async () => {
    ...
    // structure lock for applied defs
    if (!isNew && storedDef) {
    ...
    if (isNew) {
      // 연필 포크(구조 편집) — 새 정책으로 저장하고 이 패키지의 기존 정책을 대체.
      if (replaceCtx) {
        setBusy(true);
        try {
      ...
      if (choice.scope.kind === "wallets") {
        ...
        for (const address of choice.scope.addresses) {
          ...
          if (choice.applyNow) {
            ...
            // 체크 해제: 기존 동작 — 전용 폴더에 '미적용 초안'만 남기고 적용 안 함.
  ...
  const onCedarChange = (next) => {
    setCedarText(next);
    // light reparse of severity / reason from hand-typed cedar
  ...
  // LLM 탭 — 자연어 의도로 만든 FormModel 을 폼 탭에 적용하고 전환.
...
/* ── LLM 탭 — 자연어 의도 → FormModel (프로토타입: window.claude 로 생성) ── */
const LLM_EXAMPLES = [
...
// structure-only projection (strip leaf values) for the structure-lock check
