/* ────────────────────────────────────────────────────────────────────────
 * Cedar / form layer — field catalog (gloss + actions), situations helpers,
 * FormModel ⇄ Cedar-text serialisation, severity parsing. Ported from
 * cedar/form/* + editor-v9/gloss/paths.ts (curated subset).
 * ──────────────────────────────────────────────────────────────────────── */
(function () {
  "use strict";
  ...
  // ── actions (trigger picker) — grouped, Korean labels ──
  const RAW_ACTIONS = {
  ...
  // fields available for a trigger — curated gloss is action-agnostic here
  ...
  /* ── situations (AND-runs split at OR joiners) ── */
  ...
  /* ── Cedar-text serialisation (FormModel → cedar) ── */
  function fmtValue(v) {
    switch (v.kind) {
      case "bool":
  ...
  // render a node list with given parity. top-level: situations OR'd, runs AND'd
   ...
   * that reads context.custom.<X> needs a manifest that fills X (policy_rpc +
   * custom_context); we regenerate it from the model + registry. */
  ...
  // policy-server method catalog — "+ 새 보강 필드 만들기" 모달의 메서드 선택지.
  ...
  // every context.custom.<X> referenced by a leaf (LHS path or RHS field path).
  ...
  // generate the manifest (policy_rpc + custom_context) for a policy model.
  ...
  /* ── address book — resolve 0x addresses to friendly names (my wallets +
   * known tokens). Mirrors hooks/useAddressBook. ── */
  ...
  /* ── model concretisation (apply binding params over leaf values) ── */
  function applyParams(model, params) {
    if (!params) return model;
    const apply = (nodes) =>
      nodes.map((n) => {
        if (window.Cedar.isGroupNode(n)) return { ...n, conds: apply(n.conds) };
