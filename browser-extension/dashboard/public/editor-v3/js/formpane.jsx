/* ════════════════════ PolicyFormPane + ValueSheet ════════════════════ */

/* Read-only structure preview (replaces the SVG PolicyDiagram). Renders the
 * when-clause as situation cards of natural-language chips. */
...
function PolicyFormPane({ initialModel, initialManifest, valuesOnly, onChange, onValidity, resetToken, severityValue, onSeverityChange, compact }) {
  ...
  // user-defined enrichment fields — restored from saved manifest, added via modal.
  ...
  /* selection (form ↔ preview highlight) */
  ...
  /* manifest auto-gen + hand-edit override */
  ...
  const [manifestText, setManifestText] = React.useState(null); // null = use auto
  ...
  /* ── values-only (binding instance) sheet ── */
  if (valuesOnly) {
...
/* ── ValueSheet (binding instance, values only) ── */
