 ...
 *   `chrome.storage.local.get("scopeball_diag_timeouts").then(console.log)`
 */
...
const STORAGE_KEY = "scopeball_diag_timeouts";
...
export async function captureTimeout(
  ...
  console.warn(
    "[Scopeball] ⏱️ __engine::timeout DIAGNOSTICS (also saved to storage)",
    snapshot,
  );
  ...
  if (!storage) {
    console.warn(
      "[Scopeball] no storage area available — timeout diagnostics logged to console only",
    );
    return;
  }
  try {
    ...
    console.warn(
      `[Scopeball] timeout diagnostics saved → chrome.storage.local["${STORAGE_KEY}"] (${list.length} stored). Read with: chrome.storage.local.get("${STORAGE_KEY}").then(console.log)`,
    );
  } catch (err) {
    console.warn("[Scopeball] failed to persist timeout diagnostics", err);
  }
}
