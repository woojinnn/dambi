/**
 * Canonical TypeScript types + parser for the adapter registry manifest.
 *
 * See `docs/specs/2026-05-15-adapter-registry-design.md` for the full design.
 * The wire format is JSON served by the adapter-registry (nginx) container at
 * `GET /manifest.json`. The registry's `scripts/build-manifest.js` produces
 * documents conforming to this shape, and the extension SW parses them with
 * `parseAdapterManifest()` before trusting any field.
 *
 * Schema is versioned (`schema_version: 1`). When breaking the wire format,
 * bump it and gate parsing on the new version.
 */
