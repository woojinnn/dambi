...
function token(address: string, decimals = 18, is_native = false) {
  return { chain_id: 1, address, symbol: is_native ? 'ETH' : 'TKN', decimals, is_native };
}
...
describe('fetchTier1', () => {
  ...
  it('combines oracle prices, balances, and allowances', async () => {
    ...
    expect(result.balances).toEqual([{ owner: OWNER, token_key: `1:${WETH}`, balance: '100' }]);
    expect(result.allowances).toEqual([
      { owner: OWNER, token_key: `1:${WETH}`, spender: SPENDER, allowance: '50' },
    ]);
