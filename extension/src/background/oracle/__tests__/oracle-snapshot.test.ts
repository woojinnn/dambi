      ...
      storage: {
        local: {
          get: vi.fn(async (keys: string | string[]) => {
            ...
            for (const key of Array.isArray(keys) ? keys : [keys]) out[key] = storage.get(key);
            return out;
          }),
...
describe('buildOracleSnapshot', () => {
  ...
  it('returns cache hits without fetching', async () => {
    ...
    const result = await buildOracleSnapshot([{ chainId: 1, address: WETH }], fetchMock as any, 10_000);
  ...
  it('fetches misses from CoinGecko and stores them for the next call', async () => {
    const fetchMock = vi.fn(async () =>
      new Response(JSON.stringify({ [WETH]: { usd: 3500, last_updated_at: 7 } })),
    );

    const first = await buildOracleSnapshot([{ chainId: 1, address: WETH }], fetchMock as any, 10_000);
    const second = await buildOracleSnapshot([{ chainId: 1, address: WETH }], fetchMock as any, 11_000);
    expect(first[0]).toMatchObject({ token_key: `1:${WETH}`, usd_price: 3500, stale_sec: 3 });
    expect(second[0]).toMatchObject({ token_key: `1:${WETH}`, usd_price: 3500, stale_sec: 4 });
  ...
  it('uses the native price path and threads the sentinel address into token_key', async () => {
    const fetchMock = vi.fn(async (url: string) => {
      ...
      return new Response(JSON.stringify({ ethereum: { usd: 3500, last_updated_at: 8 } }));
    });
