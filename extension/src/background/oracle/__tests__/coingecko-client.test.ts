...
describe('coingecko-client', () => {
  it('returns an empty Map for empty token input without calling fetch', async () => {
    ...
    const result = await fetchUsdPrices(1, [], fetchMock as unknown as typeof fetch);
  ...
  it('fetches lowercased deduped token addresses and preserves last_updated_at metadata', async () => {
    const fetchMock = vi.fn(async (url: string) => {
      ...
      expect(parsed.searchParams.get('contract_addresses')).toBe(`${WETH},${USDC}`);
      return new Response(
        JSON.stringify({
    ...
    const result = await fetchUsdPrices(1, [WETH.toUpperCase(), USDC, WETH], fetchMock as any);
  ...
  it('returns empty Map on network or HTTP failure and never throws', async () => {
    await expect(
      fetchUsdPrices(
        1,
        [WETH],
    ...
    const httpFail = await fetchUsdPrices(
      1,
      [WETH],
      vi.fn(async () => new Response('rate limited', { status: 429 })) as any,
    );
    expect(httpFail).toEqual(new Map());
  });
  ...
  it('batches token price requests at 30 addresses per request', async () => {
    ...
    const fetchMock = vi.fn(async (url: string) => {
      const count = new URL(url).searchParams.get('contract_addresses')?.split(',').length ?? 0;
