...
const mocks = vi.hoisted(() => ({
  ...
  fallback: vi.fn((transports: unknown[]) => ({ kind: 'fallback', transports })),
  http: vi.fn((url: string, options: unknown) => ({ kind: 'http', url, options })),
}));
...
describe('rpc-client', () => {
  ...
  it('reads allowances using the matching spender for each token', async () => {
    const result = await readAllowances(1, OWNER, [TOKEN_A, TOKEN_B], [SPENDER_A, SPENDER_B]);
    expect(result).toEqual([50n, undefined]);
    expect(mocks.readContract.mock.calls[0][0].args).toEqual([OWNER, SPENDER_A]);
    expect(mocks.readContract.mock.calls[1][0].args).toEqual([OWNER, SPENDER_B]);
  });
